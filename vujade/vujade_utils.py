...
def print_info(_var, _print_var=False):
    if _print_var is True:
        print('_var: ', _var)

    if torch.is_tensor(_var) is True:
        print('type: {}, shape: {}, dtype: {}, device: {}'.format(type(_var), _var.shape, _var.dtype, _var.device))
    elif isinstance(_var, np.ndarray) is True:
        print('type: {}, shape: {}, dtype: {}'.format(type(_var), _var.shape, _var.dtype))
    else:
...
def get_pkg_version(_pkg_version):
...
def get_path(_path=None, _extension=None):
...
def makedirs(_path: str = None, _mode: int = 0o777, _exist_ok: bool = False, _print_msg: bool = False) -> None:
    ...
    try:
        ...
        if _print_msg is True:
            print('The {} folder is not newly created.'.format(_path))


def save_pkl(_var, _path=os.path.join(os.getcwd(), 'save_pkl.pickle'), _protocol=pickle.DEFAULT_PROTOCOL):
    with open(_path, 'wb') as file:
        pickle.dump(_var, file, protocol=_protocol)


def load_pkl(_path=os.path.join(os.getcwd(), 'save_pkl.pickle')):
...
def lookup(name, namespace):
    ...
    if dots > 0:
        ...
        if len(options) == 1:
            return options[0]
        if len(options) > 1:
            raise ValueError('Name conflict for %s')
        raise ValueError('%s not found as a method or class' % name)
